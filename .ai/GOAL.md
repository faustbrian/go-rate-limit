...
- Atomic statements, indexed schema, cleanup, lock-contention limits, and
  migration ownership through `go-migrations`.
...
- `go-authentication` principal integration without a reverse dependency.
- `go-authorization` remains responsible for permission decisions.
- `go-service`, `go-queue`, `go-log`, and `go-telemetry` adapters.
- Outbound vendor policies remain in `go-http-client`, which MAY consume this
