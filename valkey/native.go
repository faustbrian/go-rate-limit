...
func (store *Store) Check(ctx context.Context) error {
	...
	if err != nil {
		return fmt.Errorf("%w: server info: %w", ratelimit.ErrUnavailable, err)
	}
	major, err := valkeyMajor(info)
	if err != nil {
		return fmt.Errorf("%w: Valkey 9 or newer required: %w", ratelimit.ErrUnavailable, err)
	}
	if major < 9 {
	...
	if err != nil {
		return fmt.Errorf("%w: eviction policy: %w", ratelimit.ErrUnavailable, err)
	}
