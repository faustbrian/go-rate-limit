...
func newStore(executor executor, options Options) (*Store, error) {
	if executor == nil || options.Prefix == "" || options.Timeout <= 0 ||
...
func decodeDecision(reply []string) (ratelimit.Decision, error) {
	...
	if err != nil {
		return ratelimit.Decision{}, fmt.Errorf("%w: allowed: %w", ratelimit.ErrCorrupt, err)
	}
	remaining, err := strconv.ParseUint(reply[1], 10, 64)
	if err != nil {
		return ratelimit.Decision{}, fmt.Errorf("%w: remaining: %w", ratelimit.ErrCorrupt, err)
	}
	limit, err := strconv.ParseUint(reply[2], 10, 64)
	if err != nil {
		return ratelimit.Decision{}, fmt.Errorf("%w: limit: %w", ratelimit.ErrCorrupt, err)
	}
	resetMicros, err := strconv.ParseInt(reply[3], 10, 64)
	if err != nil {
		return ratelimit.Decision{}, fmt.Errorf("%w: reset: %w", ratelimit.ErrCorrupt, err)
	}
