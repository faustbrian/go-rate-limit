...
if redis.call('EXISTS', KEYS[1]) == 1 then
    ...
    redis.call('HSET', KEYS[1],
        ...
        'revision', revision, 'tokens', tostring(limit), 'remainder', '0',
        'last', tostring(now), 'window', '0', 'used', '0')
end
...
if algorithm == 'token_bucket' then
    ...
    redis.call('HSET', KEYS[1], 'tokens', tostring(tokens),
        'remainder', tostring(remainder), 'last', tostring(now))
elseif algorithm == 'fixed_window' then
    ...
    redis.call('HSET', KEYS[1], 'window', tostring(window), 'used', tostring(used))
elseif algorithm == 'sliding_window' then
    ...
    if cost <= remaining then
        ...
        redis.call('HSET', KEYS[1], field, tostring(current) .. ':' .. tostring(value + cost))
...
redis.call('HSET', KEYS[1], 'last', tostring(now))
redis.call('PEXPIRE', KEYS[1], ttl)
if allowed == 1 then
    return {'1', tostring(remaining), tostring(limit), tostring(reset), '0', 'allowed'}
end
return {'0', tostring(remaining), tostring(limit), tostring(reset), tostring(retry), 'limited'}
`
...
if redis.call('EXISTS', KEYS[1]) == 1 then
    ...
    redis.call('HSET', KEYS[1], 'schema', schema, 'policy_id', policy_id,
        'algorithm', 'concurrency', 'revision', revision, 'last', tostring(now))
end
local observed = tonumber(redis.call('HGET', KEYS[1], 'last'))
if now < observed then now = observed end
redis.call('HSET', KEYS[1], 'last', tostring(now))
...
if existing then
    ...
    return {'1', tostring(limit - used), tostring(limit), tostring(expires),
        '0', 'allowed', tostring(expires)}
end
local remaining = math.max(0, limit - used)
if cost > remaining then
    return {'0', tostring(remaining), tostring(limit), tostring(earliest),
        tostring(math.max(0, earliest - now)), 'limited', '0'}
end
local expires = now + lease_us
redis.call('HSET', KEYS[1], lease_field, tostring(cost) .. ':' .. tostring(expires))
redis.call('PEXPIRE', KEYS[1], ttl_ms)
return {'1', tostring(remaining - cost), tostring(limit), tostring(expires),
    '0', 'allowed', tostring(expires)}
`
