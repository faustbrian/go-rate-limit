...
func (store *Store) Acquire(ctx context.Context, request ratelimit.LeaseRequest) (ratelimit.Lease, ratelimit.Decision, error) {
	...
	if err == nil || errors.Is(err, ratelimit.ErrRejected) || errors.Is(err, ratelimit.ErrCorrupt) {
		return lease, decision, err
	}
	if errors.Is(callCtx.Err(), context.DeadlineExceeded) || errors.Is(err, context.DeadlineExceeded) {
		return ratelimit.Lease{}, ratelimit.Decision{}, fmt.Errorf("%w: %w", ratelimit.ErrDeadline, err)
	}
	return ratelimit.Lease{}, ratelimit.Decision{}, fmt.Errorf("%w: %w", ratelimit.ErrUnavailable, err)
}
...
func (store *Store) Release(ctx context.Context, lease ratelimit.Lease) error {
	...
	if err == nil || errors.Is(err, ratelimit.ErrLeaseNotFound) ||
		errors.Is(err, ratelimit.ErrLeaseNotOwned) || errors.Is(err, ratelimit.ErrCorrupt) {
		return err
	}
	if errors.Is(callCtx.Err(), context.DeadlineExceeded) || errors.Is(err, context.DeadlineExceeded) {
		return fmt.Errorf("%w: %w", ratelimit.ErrDeadline, err)
	}
