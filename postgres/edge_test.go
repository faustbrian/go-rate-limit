...
func TestStateCorruptionRollbackAndArithmeticEdges(t *testing.T) {
	...
	if _, _, _, err := mutateLease(overused, leaseRequest, "x"); !errors.Is(err, ratelimit.ErrCorrupt) {
		t.Fatalf("overused mutateLease() error = %v", err)
	}
	...
	widePolicy, err := ratelimit.NewPolicy(ratelimit.PolicySpec{
		...
		Capacity: math.MaxInt64, Period: time.Microsecond, MaxCost: 1,
	})
	if err != nil {
		t.Fatal(err)
	}
