...
The blocking benchmark gate runs three 100-operation samples and enforces the
following deliberately portable ceilings:
...
The July 17 Apple M4 Max evidence remained below 1.1 us for a hot key, 0.9 us
for the bounded 100-operation cardinality sample, and 0.7/5.2/22.5/95 us for
batch sizes 1/16/64/256. Longer steady-state reports remain useful for trend
analysis, while the generous blocking ceilings detect orders-of-magnitude
regressions without pretending that developer and CI hardware are identical.
