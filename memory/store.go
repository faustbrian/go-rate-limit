...
func New(options Options) (*Store, error) {
	if options.MaxKeys <= 0 || options.Shards <= 0 || options.Shards > options.MaxKeys {
...
func (store *Store) Admit(ctx context.Context, request ratelimit.Request) (ratelimit.Decision, error) {
	...
	current := target.getOrCreate(key, request)
...
func (store *Store) Acquire(ctx context.Context, request ratelimit.LeaseRequest) (ratelimit.Lease, ratelimit.Decision, error) {
	...
	current := target.getOrCreate(key, request.Request)
	...
	if existing, ok := current.leases[request.LeaseID]; ok {
		...
		return lease, ratelimit.Decision{
			...
			Remaining: request.Request.Policy.Limit() - used,
			Reset:     existing.expiresAt, Reason: ratelimit.ReasonAllowed,
		}, nil
	}
	if request.Request.Cost > request.Request.Policy.Limit()-used {
		...
		return ratelimit.Lease{}, ratelimit.Decision{
			...
			Remaining: request.Request.Policy.Limit() - used, Reset: reset,
	...
	return lease, ratelimit.Decision{
		Allowed: true, Limit: request.Request.Policy.Limit(),
		Remaining: request.Request.Policy.Limit() - used - request.Request.Cost,
...
func (target *shard) getOrCreate(key string, request ratelimit.Request) *state {
...
func (target *shard) evictOldest() {
	keys := make([]string, 0, len(target.states))
	for key := range target.states {
